...
class SpectrogramParser(AudioParser):
    def __init__(self, audio_conf, normalize=False, augment=False):
        ...
        self.process_mel = audio_conf['process_mel']
        self.n_mels = audio_conf['n_mels']
        ...
        self.mel_basis = librosa.filters.mel(self.sample_rate, self.n_fft, n_mels=self.n_mels)
    ...
    def parse_audio(self, audio_path):
        ...
        # STFT
        D = librosa.stft(y, n_fft=self.n_fft, hop_length=hop_length, win_length=win_length, window=self.window)
        spect = np.abs(D)

        if (self.process_mel):
            # Ver1.
            # spect = librosa.feature.melspectrogram(y=y, sr=self.sample_rate, n_mels=self.n_mels, hop_length=hop_length, n_fft=n_fft)

            # Ver2. kaitho's implementation
            spect = np.dot(self.mel_basis, spect)
...
class FeatDataset(Dataset):
    def __init__(self, manifest_filepath, labels, augment=False):
        with open(manifest_filepath) as f:
            ids = f.readlines()
        ids = [x.strip().split(',') for x in ids]
        self.ids = ids
    ...
    def __getitem__(self, index):
        ...
        feat = torch.FloatTensor(np.load(feat_path))
        transcript = self.parse_transcript(transcript_path)
...
class FeatLoader(DataLoader):
    def __init__(self, *args, **kwargs):
        """
        Creates a data loader for logMel Features.
        """
