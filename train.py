    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        for i, (data) in tqdm(enumerate(test_loader), total=len(test_loader)):
            ...
            for x in range(len(target_strings)):
                ...
                wer += decoder.wer(decoding, reference) / float(len(reference.split()))
                cer += decoder.cer(decoding, reference) / float(len(reference))
            total_cer += cer
            total_wer += wer
        ...
        if args.checkpoint:
            file_path = '%s/%s_%d_%d.pth.tar' % (save_folder, args.DB_name, args.expnum, epoch + 1)
        ...
        print('Learning rate annealed to: {lr:.9f}'.format(lr=optim_state['param_groups'][0]['lr']))
